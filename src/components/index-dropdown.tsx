...
//   };
  ...
  return (
    ...
    <DropdownMenu>
      ...
      >
        {user?.name ? (
          <p className="text-sm">{getInitials(user.name.split("@")[0])}</p>
        ) : (
      ...
      <DropdownMenuContent className="mr-5 mt-1" style={{display:"flex", justifyContent:"flex-start", alignItems:"flex-start"}}>
        <DropdownMenuGroup>
          ...
          >
            <div style={{paddingRight:"1.5rem", border:"", gap:"1rem"}} className="flex">
              <Avatar  className="h-12 w-12">
                <AvatarFallback style={{fontWeight:"600", background:"linear-gradient(180deg, midnightblue, mediumslateblue)", color:"white"}} className="text-lg">
                  {user?.name
